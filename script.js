    ...
    const recipeHTML = `
        <div class="image-gallery2">
                ...
                <div class="recipe-info">
                    <p style="font-size: large; font-weight: 900;">${
                      recipe.recipeName
                    }</p>
                    <p style="color: rgb(61, 61, 60); font-size: small;">by ${
                      recipe.name
                    }</p>
                    <button onclick="displayRecipeDetails()">Details</button>
...
function uploadFile() {
  ...
  const nameInput = document.querySelector(
    '#popup input[placeholder="Organisation Name"]'
  );
  const recipeNameInput = document.querySelector(
    '#popup input[placeholder="Food Description"]'
  );
...
function displayRecipeDetails() {
  ...
  recipeDetailsName.textContent = `Food Donation By: ${nameInput.value}`;
  recipeDetails.textContent = `Food Details: ${recipeTextarea.value}`;
...
function editRecipe(index) {
  ...
  document
    ...
    .setAttribute("data-recipe-index", index);
}
...
function confirmEdit() {
  const index = document
    .getElementById("confirmEditButton")
    .getAttribute("data-recipe-index");
